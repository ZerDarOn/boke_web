...
import { FileText, Download, Eye, File, Folder, AlertCircle } from 'lucide-react';
...
const AboutFileExplorer: React.FC = () => {
  ...
  const loadFiles = async () => {
    try {
      ...
      if (result.success && result.data) {
        const filteredFiles = result.data.filter(f => f.type === 'file');
        setFiles(filteredFiles);
        if (filteredFiles.length > 0) {
          setSelectedFilePath(filteredFiles[0].path);
          loadFileContent(filteredFiles[0].path);
        }
      } else {
  ...
  const loadFileContent = async (path: string) => {
    try {
      ...
      const result = await api.files.getContent(path);
      if (result.success && result.data) {
    ...
    <div className="w-full h-[600px] bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 shadow-sm flex rounded-lg overflow-hidden font-mono transition-colors">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    {fileType === 'markdown' ? <FileText size={16} /> : <File size={16} />}
      ...
      <div className="flex-1 bg-white dark:bg-[#0a0a0a] overflow-y-auto relative">
        {loadingContent ? (
          ...
          getFileType({ ...fileContent, path: fileContent.path, type: 'file' }) === 'markdown' ? (
