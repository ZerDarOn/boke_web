...
const TimelineArchives: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-screen py-12 relative bg-white dark:bg-[#050505] overflow-hidden transition-colors duration-300">
      ...
      {viewMode === 'timeline' && !loading && posts.length > 0 && (
        <>
              ...
              <div key={year} className="relative mb-24">
                ...
                <div className="relative z-10 flex flex-col gap-12">
                  {postsByYear[year].map((post, index) => {
                    ...
                    const isPasswordProtected = post.accessLevel === 'PASSWORD';
                    return (
                      <Link
                        key={post.id}
                        to={`/posts/${post.slug}`}
                          ...
                          <div className="relative bg-white dark:bg-white/5 p-6 border-l-2 md:border-l-0 border-gray-100 dark:border-white/10 hover:border-l-4 hover:border-neon transition-all duration-300 group-hover:-translate-y-1 rounded-sm shadow-sm">
                            ...
                            <div className={`flex flex-col gap-2 ${isLeft ? 'md:items-end' : 'md:items-start'}`}>
                              <span className="font-mono text-xs text-gray-400 flex items-center gap-2">
                                {isLeft && <span className="hidden md:inline">{formatDate(post.date)}</span>}
                                ...
                                {!isLeft && <span>{formatDate(post.date)}</span>}
                                {/* Mobile date fallback */}
                                <span className="md:hidden">{formatDate(post.date)}</span>
                                {/* Password indicator */}
                                {isPasswordProtected && <Lock size={12} className="text-amber-500" />}
                              </span>
                              ...
                              <span className="mt-2 text-xs font-mono font-bold text-ink dark:text-gray-200 uppercase flex items-center gap-1 group-hover:text-neon transition-colors">
                                Read File <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform" />
                              </span>
                            </div>
                          </div>
                        </div>
      ...
      {viewMode === 'list' && !loading && posts.length > 0 && (
        ...
        <div className="max-w-4xl mx-auto px-6 pt-12 pb-24">
          <div className="flex items-center gap-4 mb-16">
            ...
            <div>
              ...
              <p className="font-mono text-sm text-gray-400">Total {posts.length} records found.</p>
            </div>
          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="text-base font-bold text-gray-800 dark:text-gray-200 group-hover:text-neon transition-colors flex items-center gap-2">
                              {post.title}
                              {isPasswordProtected && <Lock size={14} className="text-amber-500" />}
                            </h4>
                            <div className="flex items-center gap-2 mt-1 md:hidden">
                              <span className="text-[10px] font-mono text-gray-400">{formatDate(post.date)}</span>
                            </div>
                          </div>
                        </div>
